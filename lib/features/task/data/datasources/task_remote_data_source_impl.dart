...
class TaskRemoteDataSourceImpl implements TaskRemoteDataSource {
  ...
  @override
  Future<List<TaskModel>> getLastTasks({int count = 10}) async {
    await Future.delayed(const Duration(milliseconds: 300));
    ...
    final sortedList = mockJsonList
      ..sort((a, b) =>
          DateTime.parse(b['dueTime'] as String)
              .compareTo(DateTime.parse(a['dueTime'] as String)));

    final limitedList = sortedList.take(count).toList();

    return limitedList.map((json) => TaskModel.fromJson(json)).toList();
  }
}